...
public final class LazyJavaTypeParameterDescriptor extends AbstractLazyTypeParameterDescriptor {
    ...
    private final LazyJavaResolverContext f9678c;
    ...
    private final List<KotlinType> computeNotEnhancedBounds() {
        ...
        if (upperBounds.isEmpty()) {
            SimpleType anyType = this.f9678c.getModule().getBuiltIns().getAnyType();
            Intrinsics.checkNotNullExpressionValue(anyType, "c.module.builtIns.anyType");
            SimpleType nullableAnyType = this.f9678c.getModule().getBuiltIns().getNullableAnyType();
        ...
        for (JavaClassifierType transformJavaType : iterable) {
            arrayList.add(this.f9678c.getTypeResolver().transformJavaType(transformJavaType, JavaTypeResolverKt.toAttributes$default(TypeUsage.COMMON, false, this, 1, (Object) null)));
        }
    ...
    public List<KotlinType> processBoundsWithoutCycles(List<? extends KotlinType> list) {
        ...
        return this.f9678c.getComponents().getSignatureEnhancement().enhanceTypeParameterBounds(this, list, this.f9678c);
    }
}
