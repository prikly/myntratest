...
class Huffman {
    ...
    public byte[] decode(byte[] bArr) {
        ...
        for (byte b3 : bArr) {
            ...
            i += 8;
            while (i >= 8) {
                node = node.children[(b2 >>> (i - 8)) & KotlinVersion.MAX_COMPONENT_VALUE];
        ...
        while (i > 0) {
            Node node2 = node.children[(b2 << (8 - i)) & KotlinVersion.MAX_COMPONENT_VALUE];
    ...
    private void addCode(int i, int i2, byte b2) {
        ...
        while (b2 > 8) {
            b2 = (byte) (b2 - 8);
            int i3 = (i2 >>> b2) & KotlinVersion.MAX_COMPONENT_VALUE;
