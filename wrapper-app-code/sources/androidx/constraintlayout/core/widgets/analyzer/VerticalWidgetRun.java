        ...
        if (this.dimension.readyToSolve && !this.dimension.resolved && this.dimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
            ...
            if (i3 == 2) {
                ...
                if (parent != null && parent.verticalRun.dimension.resolved) {
                    float f5 = this.widget.mMatchConstraintPercentHeight;
                    this.dimension.resolve((int) ((((float) parent.verticalRun.dimension.value) * f5) + 0.5f));
                }
            } else if (i3 == 3 && this.widget.horizontalRun.dimension.resolved) {
                int dimensionRatioSide = this.widget.getDimensionRatioSide();
                if (dimensionRatioSide == -1) {
                    f3 = (float) this.widget.horizontalRun.dimension.value;
                    f4 = this.widget.getDimensionRatio();
                } else if (dimensionRatioSide == 0) {
                    f2 = ((float) this.widget.horizontalRun.dimension.value) * this.widget.getDimensionRatio();
                    i = (int) (f2 + 0.5f);
