...
class NotificationSummaryManager {
    ...
    private static Cursor internalUpdateSummaryNotificationAfterChildRemoved(Context context, OneSignalDb oneSignalDb, String str, boolean z) {
        ...
        if (count == 0 && !str.equals(OneSignalNotificationManager.getGrouplessSummaryKey())) {
            ...
            try {
                ...
                String string = query.getString(query.getColumnIndex(OneSignalDbContract.NotificationTable.COLUMN_NAME_FULL_DATA));
                query.close();
                if (getSummaryNotificationId(oneSignalDb, str) == null) {
                    return query;
                }
    ...
    private static void restoreSummary(Context context, String str) {
        ...
        Cursor cursor = null;
        try {
            cursor = OneSignalDbHelper.getInstance(context).query(OneSignalDbContract.NotificationTable.TABLE_NAME, OSNotificationRestoreWorkManager.COLUMNS_FOR_RESTORE, "group_id = ? AND dismissed = 0 AND opened = 0 AND is_summary = 0", strArr, (String) null, (String) null, (String) null);
