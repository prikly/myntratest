...
class PagesLoader {
    ...
    private Holder getPageAndCoordsByOffset(float f2) {
        ...
        if (this.pdfView.isSwipeVertical()) {
            ...
            holder.row = MathUtils.floor(Math.abs(f3 - (this.scaledHeight * ((float) holder.page))) / this.rowHeight);
            holder.col = MathUtils.floor(this.xOffset / this.colWidth);
        } else {
            holder.page = MathUtils.floor(f3 / this.scaledWidth);
            holder.col = MathUtils.floor(Math.abs(f3 - (this.scaledWidth * ((float) holder.page))) / this.colWidth);
