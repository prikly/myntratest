...
public class PDFView extends RelativeLayout {
    ...
    public float getPositionOffset() {
        ...
        if (this.swipeVertical) {
            ...
            f2 = ((float) getPageCount()) * toCurrentScale(this.optimalPageHeight);
            i = getHeight();
        } else {
            f3 = -this.currentXOffset;
            f2 = ((float) getPageCount()) * toCurrentScale(this.optimalPageWidth);
            i = getWidth();
        }
        return MathUtils.limit(f3 / (f2 - ((float) i)), 0.0f, 1.0f);
    }

    public void setPositionOffset(float f2, boolean z) {
        if (this.swipeVertical) {
            moveTo(this.currentXOffset, ((((float) (-getPageCount())) * toCurrentScale(this.optimalPageHeight)) + ((float) getHeight())) * f2, z);
        } else {
            moveTo(((((float) (-getPageCount())) * toCurrentScale(this.optimalPageWidth)) + ((float) getWidth())) * f2, this.currentYOffset, z);
        }
        loadPageByOffset();
    }
    ...
    private void drawPart(Canvas canvas, PagePart pagePart) {
        ...
        if (!renderedBitmap.isRecycled()) {
            if (this.swipeVertical) {
                f3 = toCurrentScale(((float) pagePart.getUserPage()) * this.optimalPageHeight);
                f2 = 0.0f;
            } else {
                f2 = toCurrentScale(((float) pagePart.getUserPage()) * this.optimalPageWidth);
                f3 = 0.0f;
            }
            canvas.translate(f2, f3);
            ...
            float f4 = this.currentXOffset + f2;
    ...
    private float calculateCenterOffsetForPage(int i) {
        ...
        float f3;
        if (this.swipeVertical) {
            f2 = -(((float) i) * this.optimalPageHeight);
            width = (float) (getHeight() / 2);
            f3 = this.optimalPageHeight;
        } else {
            f2 = -(((float) i) * this.optimalPageWidth);
    ...
    private void calculateOptimalWidthAndHeight() {
        if (this.state != State.DEFAULT && getWidth() != 0) {
            ...
            float f2 = ((float) this.pageWidth) / ((float) this.pageHeight);
    ...
    public void loadPageByOffset() {
        ...
        int floor = (int) Math.floor((double) ((Math.abs(f3) + ((float) (getHeight() / 5))) / toCurrentScale(f2)));
    ...
    public void zoomWithAnimation(float f2) {
        this.animationManager.startZoomAnimation((float) (getWidth() / 2), (float) (getHeight() / 2), this.zoom, f2);
    }
